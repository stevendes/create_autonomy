...
class image_converter:
  ...
  def __init__(self):
    ...
    self.cv_image = None
    self.img_to_detect = cv2.imread('/create_ws/src/create_autonomy/ca_tools/scripts/images/stopsign.png')
    self.img_to_detect = cv2.cvtColor(self.img_to_detect, cv2.COLOR_BGR2GRAY) 
  ...
  def convert(self):
    ...
    if len(good)>MIN_MATCH_COUNT:
        src_pts = np.uint8([ kp1[m.queryIdx].pt for m in good ]).reshape(-1,1,2)
        dst_pts = np.uint8([ kp2[m.trainIdx].pt for m in good ]).reshape(-1,1,2)
        ...
        img2 = cv2.polylines(img2,[np.int32(dst)],True,255,3, cv2.LINE_AA).astype(np.uint8)

    else:
        rospy.loginfo("Not enough matches are found - %d/%d",len(good),MIN_MATCH_COUNT)
    ...
    img3 = np.uint8(cv2.drawMatches(self.img_to_detect,kp1,img2,kp2,good,None,**draw_params))
